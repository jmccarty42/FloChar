...
class Decision_Tree:
    ...
    def to_json(self, parent = "null"):
        ...
        return {'name': node_name, 'is_link': False, 'parent': parent, 'children' : [branch.to_json(parent) for branch in self.branches]}
...
class Decision_Branch:
    ...
    def to_json(self, parent):
        if type(self.subtree) != Decision_Tree:
            ...
            return {'name': self.label, 'is_link':1, 'parent': parent, 'children' : [{'name': name, 'is_link': 0, 'parent': self.label}]}
        else:
            return {'name': self.label, 'is_link': 1, 'parent': parent, 'children' : [self.subtree.to_json()]}
